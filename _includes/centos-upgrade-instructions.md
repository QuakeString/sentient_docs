...
{%- assign prev_version_href = "/docs/" | append: docsPrefix | append: "user-guide/install/upgrade-instructions/centos/v" | append: previous_version_path | append: "/" | append: platform_hash | append: previous_version_anchor -%}
{%- else -%}
...
{% else %}
```bash
wget https://github.com/sentient/sentient/releases/download/v{{ current_version_with_platform }}/sentient-{{ current_version_with_platform }}.rpm
```
{: .copy-code}
{% endif %}
...
{% if docsPrefix == "pe/" %}
* Install Sentient Web Report component as described [here](/docs/user-guide/install/pe/rhel/#step-9-install-sentient-webreport-component).
{% endif %}

```bash
sudo rpm -Uvh sentient-{{ current_version_with_platform }}.rpm
```
{: .copy-code}
...
In case action is required, you can refer to *"you will need to install Java..."* section of [**Cassandra installation guide**](/docs/user-guide/install/rhel/?rhelSentientDatabase=hybrid#cassandra-installation).
{% endcapture %}
