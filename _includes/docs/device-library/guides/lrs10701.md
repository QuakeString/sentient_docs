{% assign deviceName = "SENSO8 Indoor Air Quality (IAQ) IoT Sensor" %}
...
- Excellent platform compatibility:
  - Works well with major LoRaWAN network servers and platforms like SENTIENT, ChirpStack, The Things Stack and LORIOT.
...
LRS10701 uses LoRaWAN technology to send data to the SENTIENT platform. To start with, you need to configure the LoRaWAN gateway and network server and make sure the data from the sensor can reach the network server. This guide uses ChirpStack open-source LoRaWAN Network Server.
...
Follow the guide on [this page](/docs/user-guide/integrations/chirpstack/){:target="_blank"} to create <b>ChirpStack integration</b> on our Sentient account and configure integration on your ChirpStack application.
...
After the device has been added to ChirpStack and integration has been setup on both SENTIENT and ChirpStack, a new device will appear on the ThingBoard user interface when the device sends any data.
...
SENTIENT provides a more user-friendly way to visualize your data with dashboards.

SENTIENT allows you to create and customize your own dashboard to suit your needs.

If you want to experience the dashboard quickly, you can [download this file](/docs/devices-library/resources/dashboards/lrs10701/lrs10701_air_quality_monitor_dashboard.json){:target="_blank" download="lrs10701_air_quality_monitor_dashboard.json"} and import it to your SENTIENT to get a glimpse of the dashboard experience.

To import the dashboard, follow these steps:

{% assign checkDataInSENTIENT = '
    ===
...
{% include images-gallery.liquid showListImageTitles="true" imageCollection=checkDataInSENTIENT %}

## Conclusion

With the knowledge outlined in this guide, you can easily connect your LRS10701 LoRaWAN IAQ sensor and visualize the data on SENTIENT.

Explore the platform [documentation](/docs/{{docsPrefix}}){:target="_blank"} to learn more about the key concepts and features of SENTIENT for configuring [alarm rules](/docs/{{docsPrefix}}user-guide/device-profiles/#alarm-rules){:target="_blank"} or [dashboards](/docs/{{docsPrefix}}user-guide/dashboards/){:target="_blank"}.
