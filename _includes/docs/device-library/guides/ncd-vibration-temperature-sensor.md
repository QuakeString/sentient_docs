...
In this tutorial, we will guide you through the process of setting up the NCD Industrial IoT Wireless Vibration and Temperature Sensor with the Enterprise IIoT Gateway.This includes creating a device, configuring parameters, transmitting sensor data to SENTIENT via the MQTT protocol, and designing a real-time dashboard for sensor data monitoring and analysis.

**Enterprise IIoT Gateway Lite**

![Enterprise IIoT Gateway Lite](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/3-ncd-to-sentient-IIoT-Gateway-Lite.png){: style="float: left; max-width: 200px; max-height: 200px; margin: 0px 10px 0px 0px"}
...
{% capture difference %}
**Note**: While the SENTIENT Cloud is used in this example, [SENTIENT on-premise deployment](/docs/user-guide/install/pe/installation-options/){:target="_blank"} can also be used.
{% endcapture %}
...
With these components, you can connect the NCD sensor to the Enterprise IIoT Gateway and leverage the built-in Node-RED instance to forward sensor data to SENTIENT using the MQTT protocol.

This tutorial provides a comprehensive walkthrough of the essential configuration steps, including connecting NCD devices, registering them on the platform, setting up a SENTIENT Cloud account, adding and linking a new gateway, and creating a dashboard to visualize real-time sensor data effectively.
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/5-ncd-to-sentient-nodered.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/6-ncd-to-sentient-vibration-sensor-inside.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/7-ncd-to-sentient-debug-window.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/8-ncd-to-sentient-debug-run-message.png)
...
In this tutorial, our primary focus will be on transmitting **RUN sensor mode** and **sensor data** messages to effectively monitor the sensor’s real-time status. We will explore these topics in greater detail later in the tutorial. For now, let’s move on to **SENTIENT** to configure all the necessary components.

{% assign sentientDebugMsgPayload = '
    ===
        image: /images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/9-ncd-to-sentient-debug-msg-payload.png,
'
%}

{% include images-gallery.liquid imageCollection=sentientDebugMsgPayload %}

## Configuring SENTIENT Cloud
...
{% assign sentientWelcome = '
    ===
        image: /images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/sign-into-sentient-1-pe.png,
        title: The first step is to sign in using your SENTIENT account.
'
%}

{% include images-gallery.liquid imageCollection=sentientWelcome %}

<br>
Once you sign in, you will be presented with important resources to help you get started, along with solution templates. Take a moment to explore these to familiarize yourself with the **SENTIENT** environment.

{% assign sentientWelcome2 = '
    ===
        image: /images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/sentient-welcome-1-pe.png,
        title: Once you sign in, you will be presented with important resources to help you get started, along with solution templates. Take a moment to explore these to familiarize yourself with the SENTIENT environment.
'
%}

{% include images-gallery.liquid imageCollection=sentientWelcome2 %}

<br>
The first step is to add a new [device](/docs/{{docsPrefix}}user-guide/ui/devices/){:target="_blank"} to your SENTIENT instance. To do this, follow thi steps:
...
{% assign addingIsGatewayDevice = '
    ...
    ===
        ...
        title: After this, a new window will appear, allowing you to test the connectivity. However, since we are using a Gateway, a different set of MQTT topics is required for proper integration with the SENTIENT gateway device. You can skip this step. For now, we will identify the MQTT server to use for the connection: "mqtt.sentient.cloud". Click the "**Close**" button to proceed.
    ===
...
The next step is to **import** and **configure** a pre-configured **flow template**. This flow will allow us to filter and format the vibration sensor data before connecting it to the SENTIENT MQTT broker server and publishing the sensor data. The flow will also set the appropriate topics based on the SENTIENT MQTT Gateway API reference. To import the flow into your workspace, begin by navigating to the GitHub repository and copying or downloading the RAW JSON file: [ncd-vibration-sensor-t80-to-sentient-mqtt.json](/docs/devices-library/resources/ncd-vibration-sensor-t80-to-sentient-mqtt.json){:target="_blank"}
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/28-ncd-to-sentient-node-red-import.png)

<br>
To import the **ncd-vibration-sensor-t80-to-sentient-mqtt.json** file, simply open the window and either select the file using the "**Select a file to import**" button or paste the copied JSON code into the pink text box. Once done, click the "**Import**" button to proceed.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/29-ncd-to-sentient-node-red-import.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/30-ncd-to-sentient-node-red-importing.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/31-ncd-to-sentient-node-red-connect.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/32-ncd-to-sentient-node-red-info.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/33-ncd-to-sentient-node-red-filter.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/34-ncd-to-sentient-node-red-run-mode.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/35-ncd-to-sentient-node-red-set-device.png)
...
**ncd-set-mqtt-topic-connect**: This node sets the MQTT topic for SENTIENT, which serves to inform the platform that the device is successfully connected to the Gateway.   
**msg.topic = &#39;v1/gateway/connect&#39;**

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/36-ncd-to-sentient-node-red-topic-connect.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/37-ncd-to-sentient-node-red-filter-by-type.png)

<br>
**ncd-attributes**: This is a function node that uses JavaScript code to ensure that a sensor_data message of **type 80** is sent only once after a deployment. The node allows us to define various **sensor attributes**, such as the **address**, **sensor name**, **firmware version**, **node ID**, and **sensor type**. These attributes are then wrapped in the correct format according to the SENTIENT MQTT Gateway API. Below is the JavaScript code:
...
**ncd-set-mqtt-topic-attributes**:  This node sets the MQTT topic for SENTIENT. The topic is used to update the sensor attributes on SENTIENT.   
**msg.topic = &#39;v1/gateway/attributes&#39;**

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/38-ncd-to-sentient-node-red-topic-atributes.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/39-ncd-to-sentient-node-red-battery.png)

<br>
**ncd-telemetry**: This is a function node. The JavaScript code extracts the **Sensor MAC address** and **timestamp** data from the msg.payload message. It then retrieves the **sensor_data** message (which includes battery percentage) and wraps it in the correct format for the SENTIENT MQTT Gateway API. The JavaScript code is as follows:
...
**ncd-set-mqtt-topic-telemetry**: this node Set the MQTT SENTIENT telemetry topic, this topic serves to set the sensor data values on telemetry SENTIENT.   
**msg.topic = &#39;v1/gateway/telemetry&#39;**

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/40-ncd-to-sentient-node-red-topic-telemetry.png)

<br>
**ncd-to-sentient**: This MQTT output node establishes a connection to the MQTT Broker using the required credentials to connect to SENTIENT, allowing the data generated by the vibration sensor to be published to the SENTIENT Cloud.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/41-ncd-to-sentient-node-red-mqtt-node.png)

<br>
Once we've reviewed the general functions of each Node-RED node in the flow, the next step is to configure the MQTT out node with the credentials generated in **SENTIENT**. To access the properties of this node, double-click on the "**ncd-to-SENTIENT**" node. Once you've opened the properties, the first step is to configure the "**mqtt-broker config node**". To do this, click the "**Add new mqtt-broker config node**" button.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/42-ncd-to-sentient-node-red-add-new-mqtt.png)

<br>
The next step is to set the Server property with the SENTIENT MQTT Broker server address: "**mqtt.sentient.cloud**". For this example, we&#39;ll use the default port, **1883**. Lastly, you need to configure the **Client ID** property, which is generated in SENTIENT.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/43-ncd-to-sentient-node-red-client-id.png)

<br>
The next step is to navigate to the security tab and enter the **Username** and **Password** credentials generated in **SENTIENT**.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/44-ncd-to-sentient-node-red-security.png)

<br>
Finally, to save the configurations made within the node, click the "**Update**" button.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/45-ncd-to-sentient-node-red-update.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/46-ncd-to-sentient-node-red-mqtt-server.png)

<br>
To save the changes made within the MQTT out node, click on the "**Done**" icon.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/47-ncd-to-sentient-node-red-done.png)

<br>
Once the MQTT credentials are configured, the next time the NCD vibration sensor transmits a message, it will be published to SENTIENT via MQTT on the corresponding topic. To save and apply the changes made in your Node-RED application, click the "**Deploy**" button located at the top-right corner of the Node-RED editor.

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/48-ncd-to-sentient-node-red-deploy.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/48-ncd-to-sentient-node-red-mqtt-node.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/49-ncd-to-sentient-node-red-mqtt-node-connected.png)
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/50-ncd-to-sentient-vibration-reset-button.jpg)
...
Once a new message is transmitted by the sensor, you should be able to see some messages into the debug window that comes from the "**debug sentient**" node, you can identify this messages by the **msg.topic** property:

- **v1/gateway/connect**   
- **v1/gateway/attributes**   
- **v1/gateway/telemetry**   

![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/51-ncd-to-sentient-node-red-mqtt-messages.png)
...
If the message is a **RUN** message, the flow extracts the **MAC address** of the sensor and formats the message according to the **SENTIENT API**. This message is crucial as it informs SENTIENT that the device is successfully connected to the Gateway, allowing the platform to recognize the sensor and begin receiving telemetry and attribute data for further analysis and visualization.
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/52-ncd-to-sentient-node-red-mqtt-connect.png)

### Attributes message

To build this message, the flow uses the **sensor_data** message, which contains essential sensor data. It extracts key pieces of information such as the **sensor name**, **firmware version**, **node ID**, and **sensor type**. This data is then formatted according to the **SENTIENT attributes API**, which requires the data to be in key-value pair format. The flow sends this message only once to SENTIENT.

In SENTIENT, **device attributes** are key-value pairs that are associated with a device. These attributes are used to store important information, such as configuration settings, metadata, and operational details, providing context for the device. By including the sensor's attributes, SENTIENT can store relevant information like the sensor's name, its version, and other characteristics, which are crucial for monitoring and managing the device efficiently within the platform.
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/53-ncd-to-sentient-node-red-mqtt-attributes.png)

### Telemetry message

This message contains the real-time **sensor data**, which includes key readings such as **temperature**, **vibration**, and **battery percentage**. The flow formats this data and publishes it to **SENTIENT** as **telemetry data**. Telemetry data in SENTIENT is used for real-time monitoring and visualization, enabling users to track the sensor's status and performance over time.

By sending this telemetry message, the sensor's data is continuously updated on the SENTIENT platform, where it can be displayed on dashboards for analysis, triggering alerts, or taking other actions based on the sensor&#39;s readings. This allows for effective monitoring and management of sensor networks in industrial IoT applications.
...
![image](/images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/54-ncd-to-sentient-debug-sensor-data.png)

## Checking connection to SENTIENT Cloud

Now, return to the **SENTIENT** platform. You will notice that a new device has been automatically added, identified by its **MAC address**. Both the **Enterprise IIoT Gateway** and the **Sensor device** will show an **Active** state. The system will automatically update the device status based on the sensor&#39;s communication.
You can click on the newly added device to explore its details, **Attributes**, **Latest telemetry**, **Alarms**, **Events**, **Relations**, **Audit Logs**, among others. By reviewing these sections, you can gain deeper insights into the device's status and performance within the SENTIENT platform.
...
{% assign createdDevice1 = '
    ===
        ...
        title: Now, return to the **SENTIENT** platform. You will notice that a new device has been automatically added, identified by its **MAC address**. Both the **Enterprise IIoT Gateway** and the **Sensor device** will show an **Active** state. The system will automatically update the device status based on the sensor's communication.
'
%}
...
## Create a SENTIENT Dashboard

The next step is to create a SENTIENT [Dashboard](/docs/{{docsPrefix}}user-guide/dashboards/){:target="_blank"}. The **Dashboard** in SENTIENT allows users to visualize and monitor data collected from IoT devices. Let's create a dashboard and add some [widgets](/docs/{{docsPrefix}}user-guide/widgets/){:target="_blank"} to display a list of entities and their latest NCD Sensor values.
...
{% assign addingDashboard2 = '
    ...
    ===
        ...
        title: We will adjust the layout by selecting the "**Horizontal. Divided**" option under the "**Appearance**" properties. SENTIENT provides real-time visualization of available layout types for this widget, making it a valuable tool for fine-tuning this property. Click "**Add**";
    ===
        image: /images/devices-library/ready-to-go-devices/ncd-vibration-temperature-sensor/adding-new-dashboard-14-pe.png,
        title: Edit the size and position of the widget according to your needs. To save any changes made to your SENTIENT dashboard, you should click the "**Save**" button located in the upper right corner.
'
%}
...
This tutorial explained the basic, step-by-step process of integrating the NCD Industrial IoT Wireless Vibration and Temperature Sensor with SENTIENT Cloud, enabling you to manage and visualize the vibration and temperature variables sent by the NCD sensor.

After successfully connecting the NCD Industrial IoT Wireless Vibration and Temperature Sensor to the SENTIENT platform, you can expand its functionality by setting up thresholds and alarms for sensor data, enabling notifications via email, SMS, or web when thresholds are exceeded. You can manage users by adding them with specific roles and permissions, allowing customized access to devices and dashboards. 
