...
The [SENTIENT Edge](/docs/{{docsPrefix}}getting-started-guides/what-is-edge/){: target="_blank"} can be run alongside the [SENTIENT IoT Gateway](/docs/iot-gateway/what-is-iot-gateway/){: target="_blank"}. 
...
**SENTIENT IoT Gateway** collects data locally, translates it into the [SENTIENT-compatible communication protocols](/docs/{{peDocsPrefix}}edge/api/){: target="_blank"}, and transmits it to the **SENTIENT Edge**.
...
* The **SENTIENT Cloud** Account (Demo, Professional edition, or Community edition)
...
Create a docker-compose file for the **SENTIENT Edge** service within this directory:
...
Any connection to **localhost:8080** on your local machine will be forwarded to **127.0.0.1:8080** on the **IoT Gateway device**. The **SENTIENT Edge** instance is available at [**http://127.0.0.1:8080**](http://127.0.0.1:8080){: target="_blank"}. Use your credentials to log in.

## Step 3. Provisioning the SENTIENT IoT Gateway on Edge

To provision the **SENTIENT IoT Gateway** on **Edge** instance:

{% include images-gallery.html imageCollection="provisionGW" showListImageTitles="true" %}

## Step 4. Installing the SENTIENT IoT Gateway Software on the Gateway Device
...
To install **SENTIENT IoT Gateway**, create a new directory:
...
Create the docker-compose file for your **SENTIENT IoT Gateway** within this directory:
...
To start the **SENTIENT IoT Gateway**, execute the following command in the terminal:
...
Once the **SENTIENT Edge** and **SENTIENT IoT Gateway** are running on the **IoT Gateway device**, you can connect IoT devices and sensors to transfer data. Read [this real-life example of visualizing data from IoT devices on a Dashboard](/docs/{{docsPrefix}}device-library/recomputer-r1000/#visualize-incoming-data-with-the-dashboard){: target="_blank"} when **SENTIENT Edge** and **Gateway** are deployed on the same **IoT Gateway device**. 
