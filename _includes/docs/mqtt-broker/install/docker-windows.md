...
This guide will help you install and start standalone ST-RMQTT {{st-rmqttSuffix}} using Docker on Windows.
...
Additionally, add the following line to your `st-rmqtt-install-and-run.ps1` script (locate `create_volume_if_not_exists` lines) to create a volume for Redis data:
...
{% if docsPrefix == null %}
```shell
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/sentient/st-rmqtt/{{ site.release.broker_branch }}/msa/st-rmqtt/configs/windows/st-rmqtt-upgrade.ps1" `
-OutFile ".\st-rmqtt-upgrade.ps1"; .\st-rmqtt-upgrade.ps1
```
{: .copy-code}
{% else %}
```shell
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/sentient/st-rmqtt-pe-docker-compose/{{ site.release.broker_branch }}/basic/windows/st-rmqtt-upgrade.ps1" `
-OutFile ".\st-rmqtt-upgrade.ps1"; .\st-rmqtt-upgrade.ps1
```
{: .copy-code}
{% endif %}
