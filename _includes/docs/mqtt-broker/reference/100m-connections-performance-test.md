...
Within this article, we will show the steps taken to ensure that ST-RMQTT can reliably handle about **100M** connected clients 
...
We have deployed the ST-RMQTT cluster of 25 nodes in the [EKS](https://aws.amazon.com/eks/) 
...
For a comprehensive understanding of ST-RMQTT architecture, please refer to the subsequent [page](/docs/{{docsPrefix}}mqtt-broker/architecture/).
...
In the described scenario, ST-RMQTT cluster consistently sustains 100,000,500 connections and efficiently handles the throughput 
...
This substantial volume of data amounts to approximately 1TB, which is stored in the initial Kafka topic labeled as `st-rmqtt.msg.all`.
...
| Service Name              | **ST-RMQTT**  | **AWS RDS (PostgreSQL)** | **Kafka**   |
|---------------------------|-----------|--------------------------|-------------|
...
This substantial volume of data showcases the scalability and handling capabilities of ST-RMQTT.
...
(i.e. `publish_msg`, after [Kafka topics renaming [1]](https://github.com/sentient/st-rmqtt/commit/8871403fcfdce3489ee2a49c1505b998ceb46c3c#diff-85b2fafc998caf1c7d67f51c40f5639ac9ee0ee68379e07ad2f63b083f010f13) `st-rmqtt.msg.all`, 
...
ST-RMQTT cluster in the current configuration contains the capacity to process an even bigger load. Kafka provides reliable and highly-available processing of messages.
...
There is no direct communication between ST-RMQTT nodes that helped scale horizontally to achieve the mentioned results.
Although employing a QoS level of 0 would further elevate the message rate, our intention was to demonstrate ST-RMQTT's processing capabilities with a more practical setup.
...
ST-RMQTT is a great choice for both low and high message rates. 
...
To solve this issue, we implemented a distinct executor service dedicated to processing publish [callbacks for Kafka producers[2]](https://github.com/sentient/st-rmqtt/commit/4e8e6d8a2f9855c7df88074efc935cb7d19f593d). 
This measure effectively resolved the aforementioned disconnections.

To further enhance performance, we [eliminated the need for a specialized publishing queue[3]](https://github.com/sentient/st-rmqtt/commit/443e260924e214ae89b0158a6369b06f38801bd0) 
...
UUID generation[4]](https://github.com/sentient/st-rmqtt/commit/47e674589269bb45291f471528c54370ebdaf7ed), 
and the adoption of a mechanism for sending messages [without the need for explicit flushing[5]](https://github.com/sentient/st-rmqtt/commit/3ec317072dead5cb5355d29dd7319ccde3403d04).
...
These enhancements [6](https://github.com/sentient/st-rmqtt/commit/56ede8e5ebbaa8deafd24b7a0d4050401835ebc0), 
[7](https://github.com/sentient/st-rmqtt/commit/395d48917be0186fafbdfa12c0a9b145b66f31d2), 
[8](https://github.com/sentient/st-rmqtt/commit/fa424ffc32837b4d4aa48b890eb3bc06908a7476) 
...
To address this concern, we devised a mechanism to ensure an [even distribution of clients among the broker nodes[9]](https://github.com/sentient/tb-mqtt-perf-tests/commit/bd7649a9321f56f68303b380e634617fa0abc098), 
...
Herewith you can find total cost of ownership (TCO) calculations for ST-RMQTT deployed using AWS.
...
AWS Instance Type: 25 x m6g.metal instances (64 vCPUs AWS Graviton2 Processor, 256 GiB, EBS GP3 10GiB) to host 25 ST-RMQTT nodes. Approx. price is ~23,800 USD/month.
...
ST-RMQTT is responsible for receiving messages, distributing them among the subscribers, and optionally storing them temporarily for offline clients. 
...
To gauge the progress, we can examine the `client_session` (after [Kafka topics renaming [1]](https://github.com/sentient/st-rmqtt/commit/8871403fcfdce3489ee2a49c1505b998ceb46c3c#diff-85b2fafc998caf1c7d67f51c40f5639ac9ee0ee68379e07ad2f63b083f010f13) `st-rmqtt.client.session`) 
...
AWS instance (where ST-RMQTT nodes are deployed) monitoring shows about 45% average CPU load.
...
Lastly, let us examine the JVM state of ST-RMQTT. To accomplish this, we must forward the JMX port to establish a connection and monitor the Java applications.
...
Here is the JMX monitoring for ST-RMQTT. The broker nodes are operating steadily and without any notable issues.

{% include images-gallery.html imageCollection="broker-jmx-monitoring" %}

## How to repeat the tests

Please refer to the subsequent [installation guide](/docs/{{docsPrefix}}mqtt-broker/install/cluster/aws-cluster-setup/) to learn how to deploy ST-RMQTT on AWS.
In addition, you may explore the [branch](https://github.com/sentient/st-rmqtt/tree/100M/k8s/aws#readme) 
containing the scripts and parameters employed for running ST-RMQTT during this performance test.
Lastly, the [performance tests tool](https://github.com/sentient/tb-mqtt-perf-tests/tree/100M) available for conducting performance tests, 
...
[publishers](https://github.com/sentient/tb-mqtt-perf-tests/blob/100M/k8s/broker-tests-publishers-config.yml) and
[subscribers](https://github.com/sentient/tb-mqtt-perf-tests/blob/100M/k8s/broker-tests-subscribers-config.yml) 
to simulate the desired load.

## Conclusion

This performance test demonstrates the capabilities of ST-RMQTT cluster in efficiently
...
As a result, we anticipate publishing updated performance results for ST-RMQTT cluster in the near future.
We sincerely hope that this article be valuable to individuals evaluating ST-RMQTT and those seeking to conduct performance tests within their own environments.

Your feedback is highly appreciated, and we encourage you to stay connected with our project by following us 
on [GitHub](https://github.com/sentient/st-rmqtt) and [Twitter](https://twitter.com/sentient).

## Reference Commits

[1] - [Kafka topics renaming](https://github.com/sentient/st-rmqtt/commit/8871403fcfdce3489ee2a49c1505b998ceb46c3c#diff-85b2fafc998caf1c7d67f51c40f5639ac9ee0ee68379e07ad2f63b083f010f13).

[2] - [Kafka producer callbacks](https://github.com/sentient/st-rmqtt/commit/4e8e6d8a2f9855c7df88074efc935cb7d19f593d).

[3] - [Stop adding publish messages to queue](https://github.com/sentient/st-rmqtt/commit/443e260924e214ae89b0158a6369b06f38801bd0).

[4] - [Msg pack processing, UUID generation improvements](https://github.com/sentient/st-rmqtt/commit/47e674589269bb45291f471528c54370ebdaf7ed).

[5] - [Send messages without flush](https://github.com/sentient/st-rmqtt/commit/3ec317072dead5cb5355d29dd7319ccde3403d04).

[6] - [ClientSessionInfo object reuse](https://github.com/sentient/st-rmqtt/commit/56ede8e5ebbaa8deafd24b7a0d4050401835ebc0).

[7] - [Remove application publish msg copies](https://github.com/sentient/st-rmqtt/commit/395d48917be0186fafbdfa12c0a9b145b66f31d2).

[8] - [Bytebuf in publish msg](https://github.com/sentient/st-rmqtt/commit/fa424ffc32837b4d4aa48b890eb3bc06908a7476).

[9] - [MQTT clients even distribution among broker nodes](https://github.com/sentient/tb-mqtt-perf-tests/commit/bd7649a9321f56f68303b380e634617fa0abc098).
