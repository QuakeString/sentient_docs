...
ST-RMQTT extracts the Common Name (CN) from the client’s certificate, or any certificate in the chain to generate `credentialsId` and match it against the configured [MQTT client credentials](/docs/{{docsPrefix}}mqtt-broker/user-guide/ui/mqtt-client-credentials/).

ST-RMQTT supports two matching modes:
...
After a successful match, the client is authenticated. For performance and low-latency lookups, ST-RMQTT caches credentials in Redis, while PostgreSQL ensures persistent storage.
...
* When `enabled`, ST-RMQTT skips the validation of the Not Before and Not After dates in the certificate, meaning expired or not-yet-valid certificates will still be accepted.
...
The instructions below will describe how to connect MQTT client using X.509 Certificate to ST-RMQTT.

In particular, there are two strategies that can be used for establishing connection between the client and ST-RMQTT:

- **X.509 Certificate chain**. <br>
  Configure ST-RMQTT to trust all client certificates from a specific trust anchor (*intermediate certificate*).
...
- **X.509 Certificate.** <br> Configure ST-RMQTT to accept connections from the specific devices using pre-configured client certificates.

{% capture contenttogglespecx509 %}
X.509 Certificate chain%,%x509Chain%,%templates/mqtt-broker/ssl/st-rmqtt-certificates-chain.md%br%
X.509 Certificate%,%X509Leaf%,%templates/mqtt-broker/ssl/st-rmqtt-certificates-leaf.md%br%{% endcapture %}

{% include content-toggle.liquid content-toggle-id="st-rmqttX509" toggle-spec=contenttogglespecx509 %}
