...
In high-throughput messaging systems like ST-RMQTT, [backpressure](https://medium.com/@jayphelps/backpressure-explained-the-flow-of-data-through-software-2350b3e77ce7) 
...
ST-RMQTT is designed to effectively handle both.

ST-RMQTT uses Netty as the backbone for its MQTT communication, handling all low-level networking and I/O operations. 
...
ST-RMQTT is architected to handle virtually unlimited load from publishers. 
...
To support growing workloads, ST-RMQTT can be **horizontally scaled** by deploying multiple broker instances, distributing the load and increasing throughput capacity. 
...
ST-RMQTT offers additional mechanisms to manage incoming traffic effectively.
...
While not a reactive backpressure mechanism, rate limiting in ST-RMQTT serves as an additional layer of protection by proactively controlling the volume of incoming messages. 
It complements true backpressure mechanisms by enforcing traffic constraints before the system becomes overloaded. 
ST-RMQTT supports both cluster-wide rate limits (to control total incoming traffic) and per-client rate limits (to prevent individual publishers from overwhelming the broker). 
...
Together, TCP backpressure and configurable rate limits make ST-RMQTT highly resilient and capable of self-regulating traffic before any internal processing bottlenecks or memory pressure occur.
...
To address this, ST-RMQTT introduces a backpressure-aware delivery mechanism that detects when a Netty channel becomes non-writable and temporarily pauses message delivery.
...
ST-RMQTT uses Netty as the underlying network framework, which provides built-in support for monitoring the **writability** (`channelWritabilityChanged` event) of each channel. 
This allows ST-RMQTT to detect when a subscriber’s connection becomes overwhelmed with outbound data and apply backpressure by pausing further writes to that channel.

Netty determines writability based on **write buffer watermarks** — a pair of thresholds:

- **High Watermark**: If the outbound buffer size exceeds this threshold, the channel is marked as **non-writable**. ST-RMQTT will stop sending messages to that client until the buffer drains.
- **Low Watermark**: When the buffer size drops below this value, the channel becomes **writable** again, and ST-RMQTT resumes message delivery.
...
By leveraging this mechanism, ST-RMQTT ensures that no client connection can consume excessive memory due to unchecked message delivery. 
...
ST-RMQTT differentiates backpressure behavior based on whether the subscriber client is **persistent** or **non-persistent**, ensuring efficient use of memory and storage resources.

#### Non-Persistent Clients

For non-persistent clients, ST-RMQTT does **not store messages** if the channel becomes non-writable. Instead, when backpressure is detected:
...
ST-RMQTT ensures durability even under backpressure by using persistent storage for message queuing and controlling delivery based on channel writability.
...
Once the channel becomes writable again, ST-RMQTT resumes delivery by reading pending messages from Redis.
...
For MQTT 5.0 clients that specify a `message expiry interval`, ST-RMQTT respects the client-defined value and uses it in place of the configured default.

For **Application clients**, messages are stored in **Kafka**. If the channel to the client becomes non-writable, 
ST-RMQTT temporarily **pauses the Kafka consumer** for that client to avoid polling and buffering unnecessary messages. 
...
ST-RMQTT also applies backpressure handling logic in the context of [shared subscriptions](/docs/{{docsPrefix}}mqtt-broker/user-guide/shared-subscriptions/), ensuring reliable and efficient message delivery across all subscription types. 
...
If a subscriber in the group becomes non-writable, ST-RMQTT skips it and attempts to deliver the message to another writable subscriber in the group. 
...
When a subscriber in the group becomes non-writable, ST-RMQTT removes it from the Kafka consumer group associated with the shared subscription. 
...
This approach ensures that ST-RMQTT maintains performance, reliability, and resource efficiency even when handling shared subscriptions under pressure. Each strategy is tailored to the persistence level of the clients in the group.

## Recommendations

To maximize the effectiveness of ST-RMQTT’s backpressure handling and ensure system resilience under variable load, we recommend the following:
...
By following these practices, you can take full advantage of ST-RMQTT’s backpressure handling mechanisms, ensuring reliable operation, efficient resource usage, and high performance even in demanding MQTT workloads.

## Conclusion

The backpressure handling mechanisms in ST-RMQTT significantly enhance the broker’s resilience and efficiency when dealing with varying client consumption rates. 
...
ST-RMQTT ensures reliable performance and optimal resource utilization—even under sustained or bursty high-load conditions. 
This makes ST-RMQTT well-suited for demanding MQTT workloads at scale.
