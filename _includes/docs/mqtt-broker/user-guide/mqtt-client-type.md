...
In ST-RMQTT, two distinct types of clients are supported: **DEVICE** and **APPLICATION**. 
...
These clients are typically associated with IoT devices or sensors that frequently transmit data to ST-RMQTT.
...
By categorizing clients into these distinct types, we can better tailor ST-RMQTT to accommodate the specific 
...
All messages published by MQTT clients are persistently stored in the `st-rmqtt.msg.all` Kafka topic. 
...
ST-RMQTT employs Kafka consumer(s) that actively polls messages from the `st-rmqtt.msg.all` topic, subsequently forwarding these messages to their intended recipients. 
...
This persistence enables ST-RMQTT to ensure message delivery to the client once it reconnects. Consequently, a distinct approach is employed for message processing intended for such clients.
...
ST-RMQTT optimizes message delivery and ensures reliable communication in both persistent and non-persistent client scenarios.
...
In the case of persistent DEVICE clients available, related messages are directed to an additional Kafka topic known as `st-rmqtt.msg.persisted`.

To facilitate the delivery of messages to both online and offline clients, dedicated Kafka consumer(s) diligently polls messages from the `st-rmqtt.msg.persisted` topic. 
...
While ST-RMQTT is designed with the assumption that APPLICATION clients require persistence, this behavior is not strictly enforced.
...
Upon polling messages from the `st-rmqtt.msg.all` Kafka topic, ST-RMQTT forwards messages to a dedicated Kafka topic for each persistent APPLICATION client.
...
```
st-rmqtt.msg.app.$CLIENT_ID_HASH
```
...
> The following table summarizes the key differences between DEVICE and APPLICATION clients in ST-RMQTT, highlighting their roles, persistence behavior, message routing strategies, and typical use cases.
