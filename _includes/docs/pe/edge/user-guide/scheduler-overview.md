...
The **SENTIENT Scheduler** is a feature that allows users to automate periodic tasks, such as telemetry and attribute updates, RPC commands, and rule chain triggers.
...
Since **rule chains** and **scheduler events** cannot be created directly on the **Edge**, all configurations are first completed on the **SENTIENT Platform** and then synchronized with the **Edge** instance.
