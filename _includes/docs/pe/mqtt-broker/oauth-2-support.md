...
ST-RMQTT Professional Edition allows you to provide Single Sign-On (SSO) functionality for your users and automatically create Administrators or Viewers using external user management platforms that support the OAuth 2.0 protocol.
...
ST-RMQTT supports the Authorization Code grant type to exchange an authorization code for an access token.
Once the user is redirected back to the ST-RMQTT client, the platform retrieves the authorization code from the URL and uses it to request an access token from the external user management platform.
Using the [basic mapper](#basic-mapper) or [custom mapper](#custom-mapper), the external user info object is converted from the external platform into a ST-RMQTT internal OAuth 2.0 user.
After this, the regular ST-RMQTT authorization flow takes place.
...
Follow these steps to add a new OAuth 2.0 client to ST-RMQTT:
...
* Enter an OAuth client name and add the ST-RMQTT redirect URI in the "Authorized Redirect URIs" section using the format:
...
* Replace `domain` with your ST-RMQTT domain and specify the port used for HTTP access.
  For example, if your domain is *my.st-rmqtt.org*:

```
https://my.st-rmqtt.org/login/oauth2/code/
```
...
### Configuring Google as an OAuth 2.0 authentication provider in ST-RMQTT

To configure OAuth 2.0 authentication in ST-RMQTT via Google, follow these steps:

* Log in to your ST-RMQTT instance;
* Go to the "OAuth 2.0" page in the "Security" section;
* On the "Domains" tab, click the "plus" icon;
* Enter your domain name or IP address of your ST-RMQTT instance;
...
Now, go to the ST-RMQTT login screen. You will see a new "Login with Google" option.
Select one of your Google accounts, and you will be logged into ST-RMQTT using your Google email.
...
* Name your application "ST-RMQTT" and choose the application type **Regular Web Applications**;
...
### Configuring Auth0 as an OAuth 2.0 authentication provider in ST-RMQTT

To configure OAuth 2.0 authentication in ST-RMQTT via Auth0, follow these steps:
...
* Enter **SENTIENT** in the realm name field, and click "Create".
...
* Enter **sentient** as the client ID. Leave the client type as **OpenID Connect**. Click "Next";
* Enable the **Client authentication** option. Confirm that **Standard flow** is enabled. Click "Next";
* In the "Login settings" section, add the ST-RMQTT redirect URI in the **Authorized Redirect URIs** section using the format:
...
* Replace `domain` with your ST-RMQTT domain and specify the port used for HTTP access.
  For example, if your domain is *my.sentient.instance*:
...
Here you can find values such as **Access Token URI**, **Authorization URI**, **JSON Web Key URI**, and **User Info URI**, which are required for configuring the OAuth 2.0 client in ST-RMQTT.
...
### Configuring Keycloak as an OAuth 2.0 authentication provider in ST-RMQTT

To configure OAuth 2.0 authentication in ST-RMQTT via Keycloak, follow the steps below:
...
Go to the ST-RMQTT login screen. You will now see the option **Login with Keycloak**.
Click this button. A window will open prompting you to sign in to your Keycloak account.
Enter your Keycloak credentials and click **Sign In**. You are now logged into ST-RMQTT using Keycloak.
...
## Mapping of the external user into the ST-RMQTT internal user structure

Mapping an external user info object into a ST-RMQTT user can be achieved using the [Basic](#basic-mapper), [Custom](#custom-mapper), GitHub, and Apple mappers.

### Basic mapper

The basic mapper merges an external OAuth 2.0 user info object into the ST-RMQTT OAuth 2.0 user with a predefined set of rules.
...
* **Allow user creation** – If enabled, and the user account does not yet exist in ST-RMQTT, it will be created automatically.
  If disabled, the user will receive an *access denied* error when trying to log in with an external OAuth 2.0 provider, if no corresponding ST-RMQTT user exists.

* **Email attribute key** – The attribute key from the external OAuth 2.0 user info that will be used for the ST-RMQTT user email property.

* **First name attribute key** – The attribute key from the external OAuth 2.0 user info that will be used for the ST-RMQTT user first name property.

* **Last name attribute key** – The attribute key from the external OAuth 2.0 user info that will be used for the ST-RMQTT user last name property.
...
A custom mapper is designed as a separate microservice running alongside the ST-RMQTT microservice.
ST-RMQTT forwards all mapping requests to this microservice and expects a ST-RMQTT OAuth 2.0 user object in response.

Refer to this [base implementation](https://github.com/sentient/st-rmqtt-custom-oauth2-mapper){:target="_blank"} as a starting point for your custom mapper.
...
If ST-RMQTT is running behind a load balancer such as HAProxy, configure the balancing algorithm properly to ensure that the correct session is maintained on the ST-RMQTT instance:

```bash
backend st-rmqtt-api-backend
  ...
...
frontend http-in
  ...
  acl st-rmqtt_api_acl path_beg /api/ /swagger /webjars /v2/ /oauth2/ /login/oauth2/ # '/oauth2/ /login/oauth2/' added
  ...
```
