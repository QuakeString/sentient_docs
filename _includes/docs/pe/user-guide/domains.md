{% if docsPrefix == null %}
{% assign SENTIENT_WITH_URL = "[SENTIENT](/docs/user-guide/install/installation-options/)" %}
{% assign SENTIENT_HOST = "IP address" %}
{% assign SENTIENT_URL = "SENTIENT" %}
{% endif %}
{% if docsPrefix == "pe/" %}
{% assign SENTIENT_WITH_URL = "[SENTIENT Professional Edition](/docs/user-guide/install/pe/installation-options/)" %}
{% assign SENTIENT_HOST = "IP address" %}
{% assign SENTIENT_URL = "SENTIENT" %}
{% endif %}
{% if docsPrefix == "paas/" %}
{% assign SENTIENT_WITH_URL = "[SENTIENT Cloud](https://sentient.cloud/signup)" %}
{% assign SENTIENT_HOST = "sentient.cloud" %}
{% assign SENTIENT_URL = "SENTIENT Cloud" %}
{% endif %}
{% if docsPrefix == "paas/eu/" %}
{% assign SENTIENT_WITH_URL = "[EU SENTIENT Cloud](https://eu.sentient.cloud/signup)" %}
{% assign SENTIENT_HOST = "eu.sentient.cloud" %}
{% assign SENTIENT_URL = "EU SENTIENT Cloud" %}
{% endif %}
...
{{SENTIENT_WITH_URL}}{:target="_blank"} {% unless docsPrefix == null %}[White labeling](/docs/{{docsPrefix}}user-guide/white-labeling/){:target="_blank"} feature{% endunless %} allows you to use a custom domain to provide secure user access.
...
Once set up, you will be able to access the SENTIENT platform through your custom domain using a secure (HTTPS) connection.
{% endif %}
{% if docsPrefix == "paas/" or docsPrefix == "paas/eu/" %}
{{SENTIENT_WITH_URL}}{:target="_blank"} [White labeling](/docs/{{docsPrefix}}user-guide/white-labeling/){:target="_blank"} feature allows you to use a custom domain to provide a personalized hostname for user access.
When you register a valid domain, {{SENTIENT_URL}} automatically requests an SSL certificate and manages future certificate renewals.
...
In order to use your own host name instead of **{{SENTIENT_HOST}}** you must register it first. To do this, follow these steps:

First, on your DNS provider&#39;s website, you must add a canonical record for your domain to map it with eu.sentient.cloud{% if docsPrefix == "pe/" %}, and add **SSL certificate**{% endif %}. See [How to Create a CNAME Record For Your Domain](#how-to-create-a-cname-record) for details.
...
{% capture domain_owner_note %}
{% if docsPrefix == "pe/" %} Starting from SENTIENT version 3.9.0, adding your own domain name is available at both the Tenant level and the Customer level.{% endif %}
...
- Log in to your {{SENTIENT_WITH_URL}}{:target="_blank"} account;
{% endif %}
{% if docsPrefix == null %}
- Log in to {{SENTIENT_WITH_URL}}{:target="_blank"} account as system administrator;
{% endif %}
...
Now you can use your domain name to access {{SENTIENT_URL}} web interface and services. Try to login by entering the chosen domain name in the browser address line.
...
The SSL certificate is valid for 90 days, but no need to worry — {{SENTIENT_URL}} will automatically renew it at least 30 days before it expires.
...
Once confirmed, the domain information and associated SSL certificate will be deleted, and you will not be able to access {{SENTIENT_URL}} web interface and services using that domain.
...
For example, $YOUR_DOMAIN_NAME is `mycompany.sentient.space`:
```bash
dig mycompany.sentient.space any
```
...
; <<>> DiG 9.16.1-Ubuntu <<>> mycompany.sentient.space any
;; global options: +cmd
;; Got answer:
...
;; QUESTION SECTION:
;mydomain.sentient.online.	IN	ANY

;; ANSWER SECTION:
mycompany.sentient.space. 3600 IN	HINFO	"RFC8482" ""
...
This output shows that there are NO CNAME added to the mycompany.sentient.space domain ("ANSWER SECTION" block).

Correct output should look like that:
```bash
...
;; ANSWER SECTION:
mycompany.sentient.space. 3600 IN	CNAME	eu.sentient.cloud
...
```
...
Learn how to customize SENTIENT login page and user interface, configure your company or product logo, and adjust the color scheme, navigate to the [White labeling](/docs/{{docsPrefix}}user-guide/white-labeling/){:target="_blank"} section.