...
Please note that SENTIENT v2.2, the platform supports [**microservices**](/docs/{{docsPrefix}}reference/msa/) deployment mode.
Although microservices option is preferable for highly-available and horizontally scalable scenarios,
many SENTIENT customers find it useful to be able to start with a single SENTIENT instance and scale in the future.

We also recommend to use this mode for development and prototyping.

In the monolithic mode, all SENTIENT components are launched in a single Java Virtual Machine (JVM) and share the same OS resources.
Since SENTIENT is written in Java, the obvious advantage of the monolithic architecture is minimization of required memory to run SENTIENT.
You can launch and run SENTIENT process with 256 or 512 MB of RAM in a constrained environment.
...
For example, if the limit of OS for your SENTIENT process is 4096 file descriptors,
...
SENTIENT rule engine is responsible for processing the incoming messages with user defined logic and flow.
...
It is possible to push messages from SENTIENT to external systems via the Rule Engine.
You can push data to external system, process data and report the results of the processing back to SENTIENT for visualization.
