...
We will use special [performance-test tool](https://github.com/sentient/performance-tests/#running) that is designed to send telemetry to SENTIENT.
This tool also creates entities like devices, dashboards, etc.

#### Step 1. Launch EC2 instance.

Launch the instance in the same VPC where your target SENTIENT server is [deployed](/docs/reference/performance/setup-aws-instances/).
Make sure SENTIENT instance ports 8080 and 1883 are accessible from the test instance.
...
Login with ssh and run the commands both on Sentient and Performance test instances:
...
The command is similar to the one listed below. Don't forget to replace the value of TB_INTERNAL_IP with the private IP address of your target SENTIENT instance.
