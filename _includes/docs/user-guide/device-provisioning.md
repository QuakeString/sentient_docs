...
As a device manufacturer or firmware developer, I would like my devices to automatically provision themselves in SENTIENT.
...
From the 3.5 version, SENTIENT allows the auto-provision of new devices on authentication over MQTT using [X.509 Certificate chain](/docs/{{docsPrefix}}user-guide/certificates/).
   
## How it works?

<object width="80%" data="/images/user-guide/device-provisioning/flow.svg"></object>

The device may send a device provisioning request (Request) to the SENTIENT. The Request should always contain a Provision key and secret. 
...
The SENTIENT validates the Request and replies with the device provisioning response (Response).
...
During the validation of the Request, SENTIENT will first check the supplied *provisionDeviceKey* and *provisionDeviceSecret* to find the corresponding [Device Profile](/docs/{{docsPrefix}}user-guide/device-profiles/).
...
* **Allowing creating new devices** - checks that the device with the same name has not registered in SENTIENT yet.
...
* **Checking pre-provisioned devices** - checks that the device with the same name has been already created in SENTIENT, but hasn't been provisioned yet.
...
You can use [bulk provisioning](/docs/{{docsPrefix}}user-guide/bulk-provisioning/) to upload this list to SENTIENT. Now, devices from the list can issue provision requests, and no other devices will be able to provision themselves.

Once the provisioning is complete, SENTIENT will update *provisionState* server attribute of the device and will set it to *provisioned* value.
...
Credentials generated by the SENTIENT server%,%without%,%templates/provisioning/mqtt-without-credentials-type.md%br%
...
Credentials generated by SENTIENT server%,%without%,%templates/provisioning/http-without-credentials-type.md%br%
...
Credentials generated by SENTIENT server%,%without%,%templates/provisioning/coap-without-credentials-type.md%br%
