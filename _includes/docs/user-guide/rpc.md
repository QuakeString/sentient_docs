...
SENTIENT allows you to send Remote Procedure Calls (RPC) from server-side applications to devices and vice versa.
Basically, this feature allows you to send commands to/from devices and receive results of commands execution. 
This guide covers SENTIENT RPC capabilities. After reading this guide, you will get familiar with the following topics:
...
SENTIENT RPC feature may be divided into two types based on the originator of the remote procedure execution:device-originated and server-originated RPC.
...
SENTIENT provides an API to send RPC commands from the device.
...
Before version 3.3, SENTIENT supported **lightweight** RPC only. 
...
They lived in memory of the server, assuming that if server dies, the dashboard widget will send the same request to other SENTIENT server in the cluster.
...
Since version 3.3, SENTIENT provides support of [**persistent**](#persistent-rpc) RPC calls.
...
- **http(s)://host:port** is your SENTIENT server base URL. For example, [{{hostName}}](https://{{hostName}})
...
SENTIENT provides a convenient API to receive and process server-side RPC commands on the device.
...
SENTIENT tracks state of the persistent RPC. There are 7 available states:

{% if docsPrefix contains 'paas/' %}

* **QUEUED** - RPC was created and saved to the database; 
  No attempt to send the RPC to device yet; 
  SENTIENT will attempt to send the RPC immediately when device becomes online or if it is already online;
...
* **SENT** - SENTIENT performed attempt to send the RPC to device.
* **DELIVERED** - device confirmed that the RPC was delivered; This is the last step of processing for **one-way** RPC;   
* **SUCCESSFUL** - SENTIENT received reply for the **two-way** RPC;  
* **TIMEOUT** - SENTIENT transport layer (MQTT/CoAP/LwM2M, etc) detected timeout of the RPC delivery;
