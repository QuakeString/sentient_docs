...
In SENTIENT, SCADA symbols are crafted using [SVG](https://en.wikipedia.org/wiki/SVG){:target="_blank"} (Scalable Vector Graphics), 
...
Creating a SCADA symbol in SENTIENT involves an SVG file along with several concepts:
...
```xml
<svg xmlns="http://www.w3.org/2000/svg" xmlns:tb="https://docs.sentient.invenia.in/svg" width="100" height="100" version="1.1" viewBox="0 0 100 100">
...
<br>
Configure the "onFanClick" click action to open the SENTIENT platform&#39;s webpage in a separate browser tab:
  ...
  - **Default settings**:
    - **Action**: Open URL
    - **URL**: https://docs.sentient.invenia.in/
    - **Attribute key**: fanOn
...
The `ScadaSymbolContext` (represented as `ctx` in functions code) is a JavaScript object integral to interacting with SCADA symbols in SENTIENT. It contains the following fields:
...
The `ScadaSymbolAnimation` interface in SENTIENT is designed to facilitate complex animations within SCADA symbols using CSS animation.
...
The `ConnectorScadaSymbolAnimation` interface in SENTIENT is designed to manage animations for SVG connector elements, specifically for flow animations along a defined path. 
