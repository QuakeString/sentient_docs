...
Since **SENTIENT Edge** uses the **PostgreSQL** database for local storage, configuring **MD5 authentication** ensures that only authenticated users or applications can access the database, thus protecting your data.
...
Then run the command that will restart the **PostgreSQL** service to apply configuration changes, connect to the database as a postgres user, and create the **SENTIENT Edge database (tb_edge)**.
