...
Open the Terminal and replace $SENTIENT_HOST_NAME and $ACCESS_TOKEN with corresponding values.

```bash
mosquitto_pub -d -q 1 -h "$SENTIENT_HOST_NAME" -p "1883" -t "v1/devices/me/telemetry" -u "$ACCESS_TOKEN" -m {"temperature":25}
```
{: .copy-code}
...
```bash
mosquitto_pub -d -q 1 -h "mqtt.sentient.cloud" -p "1883" -t "v1/devices/me/telemetry" -u "ABC123" -m {"temperature":25}
```
{: .copy-code}
...
```bash
mosquitto_pub -d -q 1 -h "mqtt.eu.sentient.cloud" -p "1883" -t "v1/devices/me/telemetry" -u "ABC123" -m {"temperature":25}
```
{: .copy-code}
...
{% capture difference %}
**Note:** Since SENTIENT 3.2, you are able to use basic MQTT credentials (combination of client id, username and password)
