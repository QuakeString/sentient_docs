...
| username                  | **DEVICE_USERNAME_HERE**                     | Username for device in SENTIENT.                                            |
| password                  | **DEVICE_PASSWORD_HERE**                     | Password for device in SENTIENT.                                            |
| clientId                  | **DEVICE_CLIENT_ID_HERE**                    | Client id for device in SENTIENT.                                           |
|---
...
To communicate with SENTIENT we will use Paho MQTT module, so we should install it:
...
def collect_required_data():
    config = {}
    print("\n\n", "="*80, sep="")
    print(" "*10, "\033[1m\033[94mSENTIENT device provisioning with basic authorization example script.\033[0m", sep="")
    print("="*80, "\n\n", sep="")
    host = input("Please write your SENTIENT \033[93mhost\033[0m or leave it blank to use default ({{mqttHostName}}): ")
    config["host"] = host if host else "{{mqttHostName}}"
    port = input("Please write your SENTIENT \033[93mport\033[0m or leave it blank to use default (1883): ")
...
class ProvisionClient(Client):
    ...
    def get_new_client(self):
        ...
        if client_credentials:
            ...
            new_client.username_pw_set(client_credentials["userName"], client_credentials["password"])  # Setting username and password for SENTIENT client
...
def on_tb_connected(client, userdata, flags, rc):  # Callback for connect with received credentials
    if rc == 0:
        print("[SENTIENT client] Connected to SENTIENT with credentials: username: %s, password: %s, client id: %s" % (client._username.decode(), client._password.decode(), client._client_id.decode()))
    else:
