---
layout: device
title: How to connect Vapeless Pro Gas detection indoor Sensor to SENTIENT?
category: Other devices
connectivity: LoRaWAN
vendor: Nano Sensorics
hasIntegrationDeviceConfiguration: true
deviceImageFileName: vapeless-pro-gas-detection-indoor-sensor.png
converters20: true
model: VapelessPro
docsPrefix: pe/
hardwareType: Sensors
industry: [Security, Smart Buildings, Smart Cities]
useCase: [Smart Office, Smart Metering, Environment Monitoring, Health Care]
breadcrumbs: "true"
breadcrumbs-steps: "2"
breadcrumbs-show: "2"
---
* TOC
{:toc}

{% include get-hosts-name.html docsPrefix=page.docsPrefix %}
{% include /docs/device-library/guides/vapeless-pro-gas-detection-indoor-sensor.md %}
