---
layout: docwithnav-edge
title: Sentient Edge v4.2.x release notes
description: DRelease notes and changelog for SENTIENT GATEWAY v4.2.x
breadcrumbs: 'true'
breadcrumbs-steps: '1'
lts: February 2027
release-date: Sep 4, 2025
last-updated: Apr 1, 2026
release-note-label: AI Rule Node & Security Fixes
active-menu-item-click: 'true'
latest-patch: v4.2.2.1 (Apr 1, 2026)
effective-url: '/docs/edge/releases/releases-table/'
---

* TOC
{:toc}

{% include docs/releases/edge-releases-table/v4-2-2-1.md %}
{% include docs/releases/edge-releases-table/v4-2-2.md %}
{% include docs/releases/edge-releases-table/v4-2-1-2.md %}
{% include docs/releases/edge-releases-table/v4-2-1-1.md %}
{% include docs/releases/edge-releases-table/v4-2-1.md %}
{% include docs/releases/edge-releases-table/v4-2-0.md %}