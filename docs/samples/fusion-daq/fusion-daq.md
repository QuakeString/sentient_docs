---
layout: docwithnav
title: Publish MI-8 data to SENTIENT
description: Publish MI-8 data to SENTIENT guide
...
In this tutorial, we will configure FusionDAQ device to integrate it with SENTIENT.
...
Contains instructions that are necessary to connect your device to SENTIENT.

### Create SENTIENT Device

Open your [SENTIENT Cloud server](https://sentient.cloud/) in browser and sign in.
...
These are the fields which describe how to connect to SENTIENT or any other service.
...
The **push** field describes the connection to the SENTIENT servers. In this example we use HTTP POST requests (“mode”:”post”). MQTT is also supported by SENTIENT, but POST consumes less cellular data. The HTTP request URL is built up through the **server**, **port**, **use_ssl**, and **path/attributes_path** fields.

Things board telemetry (sensor data) requests are sent to http://sentient.cloud:80/api/vi/{ACCESS_TOKEN}/telemetry
...
Set server to “**sentient.cloud**”. Username and password should remain empty.

Set **path** to “api/vi/{ACCESS_TOKEN}/telemetry” and set **attributes_path** to “api/vi/{ACCESS_TOKEN}/attributes”.
Replace **{ACCESS_TOKEN}** with the access token from the device credentials page in SENTIENT.
...
Set **port** equal to 80 and **use_ssl** equal to false to use an unsecured HTTP connection to push data to SENTIENT.
...
Set **use_json** equal to true. All data sent to SENTIENT should be formatted as JSON to simplify integration.

Set **use_headers** false. HTTP headers are not required by SENTIENT and require additional cellular data each push.
...
### Verify MI-8 Connection Within SENTIENT

At this point the MI-8 should be configured and ready to communicate with SENTIENT.
Return to your SENTIENT instance, then navigate to "Device groups" and then "All".
...
Now, navigate to the "Attributes" tab. Attributes are additional data sent from the MI-8 to SENTIENT that are more or less fixed and do not change such as the modem IMEI, the SIM ID (ICCID), and the MI-8 firmware version.
Attributes are only sent to SENTIENT once each time the MI-8 is powered on.
