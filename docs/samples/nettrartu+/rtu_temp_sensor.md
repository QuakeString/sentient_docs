...
description: SENTIENT IoT Platform sample for temperature upload over MQTT using Nettra RTU
hidetoc: "true"
---
...
This guide contains step-by-step instructions on how to connect your Nettra RTU device to SENTIENT through ethernet, using as sample, one of the many applications that the Nettra RTU has. In particular, this sample application will allow you to monitor temperature using SENTIENT web UI to display the data coming from the sensor. 
...
Sign in to your SENTIENT server Web UI.
...
Use import/export [instructions](/docs/user-guide/ui/dashboards/#iot-dashboard-importexport) to import the dashboard to your SENTIENT instance.
...
3. Fill the ***MQTT*** boxes as follow:
   ...
   **Servidor** : SENTIENT HOST/IP address that is accessible within your local network. Specify `mqtt.sentient.cloud` if you are using [SENTIENT Cloud](https://sentient.cloud/) server.<br>
...
4. Fill the ***Time Synchronization*** boxes as follow:
   ...
   **Servidor** : SENTIENT HOST/IP address that is accessible within your local network. Specify `sentient.cloud` if you are using [SENTIENT Cloud](https://sentient.cloud/) server.<br>
   **Frecuencia** : 10<br>
...
Finally, open SENTIENT Web UI with same user and password as *SENTIENT configuration* section.
