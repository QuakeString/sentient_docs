---
layout: docwithnav
title: A Guide to Connecting ADAM-6717 to SENTIENT Cloud Unlocking Seamless IoT Connectivity
description: A Guide to Connecting ADAM-6717 to SENTIENT Cloud Unlocking Seamless IoT Connectivity
---
...
In this comprehensive guide, we will walk you through the process of establishing a seamless connection between the ADAM-6717 and SENTIENT Cloud. 
By following these step-by-step instructions, you will be able to integrate and leverage the powerful capabilities of SENTIENT Cloud with your ADAM-6717 module

## Step 1: Creating a New Device on the SENTIENT Cloud Platform

To establish a connection between your ADAM-6717 module and the SENTIENT Cloud platform, the first step is to create a new device. 
...
By creating a new device, you are preparing a destination where the data from your ADAM-6717 module will be sent for storage and visualization within the SENTIENT Cloud platform.
...
## Step 3: Configuring NodeRED for Integration with SENTIENT Cloud

This configuration sets up a flow where data will be injected by the inject node and sent to SENTIENT Cloud using the MQTT out node.
 ...
 - Inject Node: Sentient gets data in JSON format, which uses “key” and “value”. By configuring the inject node to send data in JSON format using key-value pairs, you ensure that the data is compatible with SENTIENT's data structure.
...
In the MQTT server settings, you will need to configure the following information:
 ...
 - **MQTT Broker**: Provide the address or hostname of the MQTT broker. This   is   typically   the   URL   provided   by   SENTIENT   Cloud   for   MQTT communication.
 - **Port**: Specify the port number for the MQTT broker. The default port for MQTT is 1883, but check with SENTIENT Cloud for any specific port requirements.
 - **Client ID**: Enter a unique identifier for the client connecting to the MQTT broker. This helps identify your Node-RED instance within the SENTIENT Cloud platform.
 - **Topic**: Specify the MQTT topic to which you want to publish the data. This topic should align with the topic structure defined in SENTIENT Cloud.
...
After configuring the inject node and MQTT out node as mentioned in the previous steps, you are ready to send the data. 
 ...
 - Click the "inject" button to trigger the sending of data to your device in SENTIENT Cloud.
 - Once the "inject" button is clicked, Node-RED will generate and send the data payload to the MQTT out node, which in turn will publish the data to the specified topic on the SENTIENT Cloud platform.

![image](/images/samples/solandtec/Imagen11.png)

<br>
Congratulations! You have now successfully sent the data from Node-RED to your device in SENTIENT Cloud.
