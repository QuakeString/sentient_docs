...
description: SENTIENT ANALYTICS Cache settings 
---

* TOC
{:toc}

SENTIENT ANALYTICS has many built-in optimization mechanisms for improving report generation time. Most of them work out of the box 
and do not require any configuration. But some of those mechanisms should be enabled explicitly by the administrator.

## SENTIENT ANALYTICS cache levels

SENTIENT ANALYTICS supports 2 cache levels - `View Report` cache and `Metrics long term cache`. To explain the difference between them 
we first need to understand how reports are generated in SENTIENT ANALYTICS:

1. User creates View configuration, adds required fields, and press Build Report.
2. SENTIENT ANALYTICS generates a query execution plan based on view configuration. Query plan contains fields load order and aggregation strategy.
...
value from cache instead of reloading data from SENTIENT. 
...
If field value is already in cache - system will reuse it. If it is not there yet, system will load data directly from SENTIENT and save it in the cache.
...
SENTIENT ANALYTICS cache contains aggregated field data for specific time intervals. User can define what time interval used for data caching.
...
in cache yet, the system will load data directly from SENTIENT and as a result - the user will wait few minutes while the report is ready.
...
During cache refresh in the background, SENTIENT ANALYTICS will make requests to the SENTIENT. All such requests should be authenticated and signed with JWT token.
For enabling such authentication, administrator should define login\password pair in SENTIENT ANALYTICS configuration file - `ADMIN_LOGIN` and `ADMIN_PASSWORD`. 
...
You can clear long term cache if required. It may be useful if you re-import or change historical data in SENTIENT. 
