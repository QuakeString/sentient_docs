...
assignees:
- vparomskiy
title: Save calculated values as a telemetry into SENTIENT
description: Save calculated values as a telemetry into SENTIENT

save-calc-to-tb:
  0:
    image: /images/sentient-analytics/save-calc-to-tb-1.png
    text: Click on the JOBS button to open the Parameters for Saving Calculation Telemetry Job modal.
  1: 
    image: /images/sentient-analytics/save-calc-to-tb-2.png
    text: 1. Enable/disable saving calculation results to SENTIENT. 2. Specify the starting point for saving the calculated telemetry. 3. Define how often the telemetry should be updated. 4. Select the specific devices for which the calculated telemetry will be saved.
---

Calculated fields in SENTIENT ANALYTICS offer the ability to compute various metrics based on existing telemetry and attributes.
...
What's more, SENTIENT ANALYTICS allows you to save these calculated metrics as device or asset telemetry back to SENTIENT, enhancing the functionality of your IoT application.
...
* Performance Optimization - Once the metrics are computed and saved as telemetry, you can optimize the performance of the SENTIENT ANALYTICS server.
  Instead of loading raw telemetry and recomputing metrics each time when they are required, the pre-computed metrics can be loaded directly from SENTIENT.
  This reduces the load on the server, resulting in faster and more efficient operations.

## Calculation Jobs

When you save a calculated field, it can be activated to initiate two main tasks and store the data in SENTIENT as telemetry:
...
* In the modal, you can configure the following parameters:
  1. Enable/disable saving calculation results to SENTIENT.
...
When these settings are saved, your calculations are recorded in SENTIENT as telemetry with a key name prefixed by \_ECD_, and the status of the calculation is set to “enabled”
...
![image](/images/sentient-analytics/calculations/name-calculation-field.png)
