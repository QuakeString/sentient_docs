...
sentient-analytics-models-page:
  0:
    image: /images/sentient-analytics/create-model.png
...
sentient-analytics-accuracy-page:
 0:
  image: /images/sentient-analytics/accuracy-summary.png
...
sentient-analytics-accuracy-real-data:
 0:
  image: /images/sentient-analytics/accuracy-real-data.png
...
sentient-analytics-accuracy-confidence-band:
 0:
  image: /images/sentient-analytics/accuracy-confidence-band.png
...
sentient-analytics-confidence-level:
 0:
  image: /images/sentient-analytics/accuracy-confidence-level.png
  title: This chart shows the prediction’s accuracy as a binary feature.

sentient-analytics-forecasting-energy-example:
 0:
  image: /images/sentient-analytics/prediction-config-example.png
 1:
  image: /images/sentient-analytics/prediction-accuracy-example.png
  
sentient-analytics-prediction-job-example:
 0: 
  image: /images/sentient-analytics/prediction-jobs-example.png


sentient-analytics-prediction-line-chart-example:
 0: 
  image: /images/sentient-analytics/prediction-line-chart-example.png
---

* TOC
{:toc}

SENTIENT ANALYTICS provides powerful built-in tools for time-series prediction, allowing users to create predictive models with minimal effort and data science background. All the complex processes—such as data filtering, normalization, and model training—are handled automatically in the background, simplifying the user experience.

With SENTIENT ANALYTICS, you can enable predictions for any data fields, including calculated fields. Time-series prediction opens up a wide range of possibilities for extracting valuable insights from your data. Below are some key questions that SENTIENT ANALYTICS time-series prediction can help you answer:
...
To access the prediction models tool, click on the icon![image](/images/sentient-analytics/models-icon.png)labeled “Prediction Models” located on the left side of the workspace.
...
{% include images-gallery.html imageCollection="sentient-analytics-models-page" %}

## Input tab

The Input tab in SENTIENT ANALYTICS prediction models is designed to configure all necessary settings for generating accurate forecasts. Below is a detailed explanation of each section and its settings:
...
**Key:** The identifier under which the prediction telemetry data is saved in SENTIENT with prefix _EPD_. For example if we set key as *energy_forecast* the telemetry will be saved in SENTIENT under *_EPD_energy_forecast* key.
...
* *Prediction Method:* Select the prediction method to use. Each method includes a short description to help you choose the most suitable approach for your use case. SENTIENT ANALYTICS implements different multivariable and univariable ML models for timeseries prediction:
...
* *Custom Model* - you can write our own multivariable time series prediction model using any Python libraries. In this case you provide a model source and SENTIENT ANALYTICS is responsible for inserting input dataset from SENTIENT and process forecasted output.

**Segment Strategy:**  SENTIENT ANALYTICS prediction models perform specific data preprocessing to align input telemetry data into the required format. This process includes filling gaps, aggregating data by a specified time unit, and applying aggregation functions.
...
Several strategies in SENTIENT ANALYTICS define how data is segmented:
1. **AUTO**
...
**Include Last Unfinished Segment**: For each segment strategy, you have the option to enable the inclusion of the last unfinished segment. When SENTIENT ANALYTICS divides the input telemetry into a set of segments, some data is not enough to fit the segment because the segment's time range is bigger than the remaining data time range. 
This binary property tells SENTIENT ANALYTICS whether to use the remaining telemetry to create partially filled segments.

*For example*, we have the telemetry with a range of **20 days**, a prediction range of **10 days**, and the "Sliding Windows Unit" strategy with a **1-day step**. It means SENTIENT ANALYTICS will create the first 11 segments as usual: (1-10), (2-11), (3-12), ..., (11-20). But if we continue this process each next segment will not have 10 days, 
it would decrease by 1 day for each step: (12-20), (13-20),..., (19-20). These segments can be not appropriate for your case because of overfitting on the same data without newer ones. Setting the boolean property manages the behavior of SENTIENT ANALYTICS in this case - you can allow or forbid usage of this kind of segment.
...
![image](/images/sentient-analytics/models-advanced-settings.png)

## Accuracy Tab

With SENTIENT ANALYTICS, you can not only generate predictions for a specific period but also validate the accuracy of those predictions. The Accuracy Metrics tab provides valuable insights into the performance of your prediction models using the following metrics:
1. Real Data
2. Confidence Band
3. Confidence Level
...
![image](/images/sentient-analytics/accuracy-config.png)

By carefully configuring these settings, you can tailor SENTIENT ANALYTICS prediction models to generate precise and actionable forecasts based on your specific requirements. 
...
{% include images-gallery.html imageCollection="sentient-analytics-accuracy-page" %}
...
{% include images-gallery.html imageCollection="sentient-analytics-accuracy-real-data" %}
...
{% include images-gallery.html imageCollection="sentient-analytics-confidence-level" %}
...
![image](/images/sentient-analytics/accuracy-confidence-level-param.png)
...
{% include images-gallery.html imageCollection="sentient-analytics-accuracy-confidence-band" %}
...
![image](/images/sentient-analytics/accuracy-confidence-band-param.png)
...
The Tasks tab provides an overview of all tasks initiated for a specific model, including their status (completed, in progress, or pending). For more detailed task management information, refer to the [corresponding section in the documentation](/docs/sentient-analytics/tasks-service).

## Example: Forecasting Energy Consumption for a Building

In this example, we’ll create a forecast to predict how much energy a building will consume over the next 3 month. The dataset includes Buildings, Apartments, and energy meters installed in each Apartment. The process involves aggregating telemetry from sensors at the building level and then generating a forecast using SENTIENT ANALYTICS prediction models.
...
{% include images-gallery.html imageCollection="sentient-analytics-forecasting-energy-example" %}

### Saving telemetry to SENTIENT

* If the accuracy is satisfactory, and you want to use this prediction as new telemetry, you need to record it as telemetry  in SENTIENT. To do this:
...
{% include images-gallery.html imageCollection="sentient-analytics-prediction-job-example" %}
...
{% include images-gallery.html imageCollection="sentient-analytics-prediction-line-chart-example" %}
