...
This guide describes how SENTIENT ANALYTICS uses entities from SENTIENT, like asset, device, relation, etc.
...
Now let's check how SENTIENT ANALYTICS resolves data from SENTIENT using following report: we are using only 2 fields from Smart Building topology: 
...
* SENTIENT ANALYTICS will find all available buildings in the SENTIENT. 
* Then all Apartments for each Building.
* Finally, all Energy Meters that belong to the apartment.
* After that, for all Energy Meters for each building, SENTIENT ANALYTICS will load all energy telemetry for the last month 
* SENTIENT ANALYTICS aggregates all loaded telemetry using `SUM` aggregation. 
...
SENTIENT allows users to create multiple relationships between devices and assets without limitations. In SENTIENT ANALYTICS, however, relationships are established between business entities (device/asset profiles) based on the underlying SENTIENT relations. 
This approach introduces some constraints on how relations are handled within SENTIENT ANALYTICS, specifically regarding the number of allowable relations between device/asset profiles.

In SENTIENT ANALYTICS, a key constraint exists: **only one relation type is permitted between device/asset profiles**. This limitation is necessary to ensure a stable sequence of item loading. 
When there are multiple relations between the same device/asset profiles, SENTIENT ANALYTICS is unable to determine which relation to use, leading to an undefined behavior that impacts data retrieval.
...
In this scenario, if a query is made to retrieve all `EnergyMeters` associated with a particular `Building`, SENTIENT ANALYTICS would face uncertainty in determining which `EnergyMeters` to include in the result set—those connected directly to the `Building` or those that the `Building` contains through the intermediary `Apartment`. 
...
![image](/images/sentient-analytics/be-enable-relation.png)
...
By following these steps, you can eliminate uncertainty and stabilize data queries in SENTIENT ANALYTICS, making use of the “Enabled” checkbox feature to manage relationships effectively.
