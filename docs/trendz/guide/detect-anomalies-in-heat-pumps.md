...
heatpumps-dashboard:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_detection_dashboard.png
    title: 'Dashboard for investigating discovered anomalies in Heat Pumps operation'

heatpumps-create-anomaly-model:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_create_anomaly_model.png
    title: 'Click create Anomaly detection model button'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_configure_anomaly_model.png
    title: 'Set Anomaly detection model parameters'
  2:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomalies_summary.png
    title: 'Review results of anomaly detection model training for heat pumps'
  3:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_review_anomaly.png
    title: 'Review discovered anomalies for heat pump'

heatpumps-autodiscovery-anomalies:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpumps_autodiscover_enable.png
    title: 'Click Anomaly automated discovery button'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpumps_autodiscover_job_configuration.png
    title: 'Configure intervals for automated anomaly discovery job'

heatpumps-create-anomaly-review-widget:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_create_anomaly_widget.png
    title: 'Create anomaly review widget for heat pumps'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_select_model.png
    title: 'Select anomaly detection model for heat pumps create in Step 1'
  2:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_review_widget.png
    title: 'Review discovered anomalies for heat pumps on a widget'

heatpumps-save-anomaly-score-as-telemetry:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_table_create.png
    title: 'Create anomaly score table for heat pumps based on discovered anomalies'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_table_row_click.png
    title: 'Select heat pump as a row click entity for anomaly score table'
  2:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_save.png
    title: 'Schedule anomaly score saving job for heat pumps'

heatpumps-create-anomaly-alert:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_alert_create.png
    title: 'Create anomaly score alert for heat pumps in SENTIENT device profile'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_alert_configuration.png
    title: 'Configure anomaly score alert create and clear rules'
  2:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_alert_creation_condition.png
    title: 'Configure anomaly score threshold for alert creation'
  3:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_score_alert_clear_condition.png
    title: 'Configure anomaly score threshold for alert clearing'

heatpumps-notify-about-anomalies:
  0:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_open_root_rulechain.png
    title: 'Open root rule chain in SENTIENT'
  1:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_send_email_node_configuration.png
    title: 'Add toEmail rule node to root rule chain'
  2:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_email_notification_config.png
    title: 'Configure email properties for notification'
  3:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_add_send_email_node.png
    title: 'Add send email node to root rule chain'
  4:
    image: /images/sentient-analytics/guide/heatpump_anomalies/heatpump_anomaly_send_notification_rulechain.png
...
### Prerequisites
Heat pump reports telemetry data to SENTIENT via MQTT protocol. They registered in platform as a device entity. Devices have relation to Apartment asset and apartments assigned to customer account. 
...
To identify possible issues with heat pumps, we will use SENTIENT ANALYTICS anomaly detection instruments. SENTIENT ANALYTICS uses unsupervised machine learning algorithms to detect anomalies in time series data. 
...
During model creation SENTIENT ANALYTICS will fetch all required data for the model, analyse and train to detect what is normal behavior for a heat pump. Based on this information anomaly detection model will be created.
...
Once configuration saved SENTIENT ANALYTICS will periodically fetch new data from heat pumps and identify anomalies. If anomaly was detected, SENTIENT ANALYTICS will compute anomaly score and save it in the database.
...
#### Save current anomaly score for heat pump as a telemetry back to SENTIENT
...
* Add Anomaly field into Columns section
  ...
  * Aggregation - `MAX`
  * Label - `heatpumpAnomalyScore` - SENTIENT ANALYTICS will save anomaly score index as telemetry with this name.
...
* Open view settings and enable telemetry save in `Tb calculated telemetry save` section.
  ...
  * Interval - 1
  * Unit - hours
* In settings open `View mode fields` section and **heatPump** entity in `Row click entity` dropdown - this step tells SENTIENT ANALYTICS under what entity score index telemetry should be saved.
* Set default time range to **Last 7 days**
* Save view with name **Heatpump anomaly score save job**

Once view saved, SENTIENT ANALYTICS would schedule background job that will periodically check heat pumps anomaly score and save result as telemetry of the heat pump device.
...
At this moment we already have `heatpumpAnomalyScore` telemetry for each heat pump in the SENTIENT which tells as how abnormal its current behavior. It means that we can create Alarm Rule in ThignsBoard to raise an alarm if score index is higher than 50.

* In SENTIENT open heat pump's device profile and add new Alarm Rule
...
With this configuration SENTIENT will send notification to the maintenance team once anomaly was detected in heat pumps behavior.
