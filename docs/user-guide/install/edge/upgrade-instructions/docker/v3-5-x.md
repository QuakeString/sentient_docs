...
title: SENTIENT GATEWAY CE v3.5.x upgrade instructions for Docker
description: SENTIENT GATEWAY CE v3.5.x upgrade guide for Docker
...
{% assign updateServerLink = "docker/v3-5-x/#upgrading-sentient-ce-to-351" %}
...
{% assign updateServerLink = "docker/v3-5-x/#upgrading-sentient-ce-to-350" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% capture edge-previousVersion %}
These steps are applicable for Edge {{previousVersion}} version.
{% endcapture %}
...
services:
  mytbedge:
    restart: on-failure
    image: "sentient/tb-edge:3.5.0EDGE"
    environment:
      SPRING_DATASOURCE_URL: jdbc:postgresql://postgres:5432/tb-edge
    volumes:
      - ~/.mytb-edge-data:/data
