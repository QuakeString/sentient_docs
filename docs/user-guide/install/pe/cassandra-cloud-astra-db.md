---
layout: docwithnav
title: Connect SENTIENT to Datastax Astra DB 
description: Connect SENTIENT to Datastax Astra DB
hidetoc: "true"
---

This guide describes how to setup Datastax Astra DB cloud for your SENTIENT. This is an alternative having your own Cassandra cluster.
...
Create a new database `sentient` with keyspace `sentient`. Select the closest location to you and cloud provider you like. 
...
You need a `Database administrator` role to install the Sentient schema. Click `Create a token` on the `sentient` database.
...
The `secure-connect-sentient.zip` file contains the security certificates and credentials for your database

### Step 5. Set connection parameters on SENTIENT

Here the environments you need to set to use your Astra DB with SENTIENT.

#### SENTIENT service

For SENTIENT as a local service add to the `/etc/sentient/conf/sentient.conf`

```bash
export DATABASE_TS_TYPE=cassandra
# Cassandra on Astra DB cloud 
export CASSANDRA_CLOUD_SECURE_BUNDLE_PATH=/etc/sentient/astra/secure-connect-sentient.zip
# dbadmin
...
services:
  tb:
    image: "sentient/tb-postgres:3.4.2"
    network_mode: "host"
    restart: "always"
    # replace /home/your_user/Downloads/ with your local path
    volumes:
      - /home/your_user/Downloads/secure-connect-sentient.zip:/etc/sentient/astra/secure-connect-sentient.zip
    environment:
      HTTP_BIND_PORT: "8080"
      DATABASE_TS_TYPE: "cassandra"
      # Cassandra on cloud
      CASSANDRA_CLOUD_SECURE_BUNDLE_PATH: "/etc/sentient/astra/secure-connect-sentient.zip"
      # dbadmin
...
```bash
base64 -w 0 /home/your_user/projects/astra/secure-connect-sentient.zip | xclip -selection clipboard
```
{: .copy-code}

The resulting file will look like `astra-secret.yml`. The `secure-connect-sentient.zip` encoded string is very long.
```yaml
apiVersion: v1
kind: Secret
metadata:
...
Then mount `secure-connect-sentient.zip` from secret to the `tb-node.yml` and `tb-rule-engine.yml`.
      ...
      volumes:
        - name: astra-secret
          secret:
            ...
            items:
              - key: secure-connect-sentient.zip
                path: secure-connect-sentient.zip
      containers:
        - name: tb-node
          image: sentient/tb-node:3.4.2
          volumeMounts:
            - mountPath: /etc/sentient/astra
              name: astra-secret
              readOnly: true
          env:
            ...
            - name: CASSANDRA_CLOUD_SECURE_BUNDLE_PATH
              value: "/etc/sentient/astra/secure-connect-sentient.zip"
...
### Step 6. Run standard SENTIENT install

SENTIENT is already support install on Astra DB cloud.
...
### Sentient test run on Astra DB cloud

Here an example how to test your Sentient with your Astra DB using performance test tool.
To spin up the Sentient locally, use `docker-compose.yml` provided couple sections above.
...
docker run -it --rm --network host --name tb-perf-test \
           ...
           --env REST_USERNAME=tenant@sentient.org \
           ...
           sentient/tb-ce-performance-test:latest
```
{: .copy-code}
