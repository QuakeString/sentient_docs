...
title: SENTIENT GATEWAY PE v3.5.x upgrade instructions for Docker
description: SENTIENT GATEWAY PE v3.5.x upgrade guide for Docker
...
{% assign updateServerLink = "docker/v3-5-x/#upgrading-sentient-pe-to-351" %}
...
services:
  mytbedge:
    restart: on-failure
    image: "sentient/tb-edge-pe:3.5.0EDGE"
    environment:
      SPRING_DATASOURCE_URL: jdbc:postgresql://postgres:5432/tb-edge
    volumes:
      - ~/.mytb-edge-data:/data
