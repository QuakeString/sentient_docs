...
title: SENTIENT GATEWAY PE v3.6.x upgrade instructions for Docker
description: SENTIENT GATEWAY PE v3.6.x upgrade guide for Docker
...
{% assign updateServerLink = "docker/v3-6-x/#upgrading-sentient-pe-to-364" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% assign minorVersion = "3.6.3" %}
{% assign mainteneceVersion = "3.6.4.x" %}
...
{% assign updateServerLink = "docker/v3-6-x/#upgrading-sentient-pe-to-363" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% assign minorVersion = "3.6.2" %}
{% assign mainteneceVersion = "3.6.3.x" %}
{% include templates/edge/user-guide/start-upgrade-pe.md %}
...
{% assign updateServerLink = "docker/v3-6-x/#upgrading-sentient-pe-to-362" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
...
Starting with the {{versionName}} release, the SENTIENT team has moved from using Docker bind mount folders to Docker volumes.
...
Modify the `docker-compose.yml' file for SENTIENT GATEWAY to update the volume settings.
...
The next step creates a docker compose file for the **SENTIENT GATEWAY upgrade** process and runs the upgrade. Once the upgrade process is successfully completed, the TB Edge upgrade container is automatically stopped:
...
services:
  mytbedge:
    restart: on-failure
    image: "sentient/tb-edge-pe:{{versionName}}"
    environment:
...
{% assign updateServerLink = "docker/v3-6-x/#upgrading-sentient-pe-to-361" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% capture edge-previousVersion %}
These steps are applicable for Edge {{previousVersion}} version.
...
```
docker pull sentient/tb-edge-pe:{{versionName}}
```
{: .copy-code}
...
The next step creates a docker compose file for the **SENTIENT GATEWAY upgrade** process and runs the upgrade. 
...
{% assign updateServerLink = "docker/v3-6-x/#upgrading-sentient-pe-to-360" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
