---
layout: docwithnav-pe-edge
title: SENTIENT GATEWAY PE v3.8.x upgrade instructions for Docker
description: SENTIENT GATEWAY PE v3.8.x upgrade guide for Docker
active-menu-item-click: "true"
breadcrumbs: true
breadcrumbs-steps: 2
breadcrumbs-show: 2
effective-url: '/docs/user-guide/install/pe/edge/upgrade-instructions/'
---

* TOC
{:toc}

{% include templates/edge/install/docker-backup-before-upgrade.md %}

## Upgrading Edge PE to 3.8.0

{% assign versionName = "3.8.0EDGEPE" %}
{% assign previousVersion = "3.7.0EDGEPE" %}
{% assign updateServerLink = "docker/v3-8-x/#upgrading-sentient-pe-to-380" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% assign minorVersion = "3.7.0" %}
{% assign mainteneceVersion = "3.8.0.x" %}
{% include templates/edge/user-guide/start-upgrade-pe.md %}

