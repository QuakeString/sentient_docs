---
layout: docwithnav-pe-edge
title: SENTIENT GATEWAY PE v4.0.x upgrade instructions for Docker
description: SENTIENT GATEWAY PE v4.0.x upgrade guide for Docker
active-menu-item-click: "true"
breadcrumbs: true
breadcrumbs-steps: 2
breadcrumbs-show: 2
effective-url: '/docs/user-guide/install/pe/edge/upgrade-instructions/'
---

* TOC
{:toc}

{% include templates/edge/install/docker-backup-before-upgrade.md %}

## Upgrading Edge PE to 4.0.1

{% assign versionName = "4.0.1EDGEPE" %}
{% assign previousVersion = "3.9.1EDGEPE" %}
{% assign updateServerLink = "docker/v4-0-x/#upgrading-sentient-pe-to-401" %}
{% assign peDocsPrefix = "pe/" %}
{% include templates/edge/install/compatibility-warning-version.md %}
{% assign minorVersion = "3.9.1" %}
{% assign mainteneceVersion = "4.0.1.x" %}
{% include templates/edge/user-guide/start-upgrade-pe.md %}



