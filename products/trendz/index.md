---
layout: sentient-analytics
title: IoT Anomaly Detection & Predictive Maintenance using AI
description: IoT solution for real-time data analysis, anomaly detection, predictive maintenance, and forecasting. Predict asset RUL & time to failure and find root causes.

---

<section class="pe-features sentient-analytics-features">
    <div>
        <main>
            <div class="content-list">
                <div class="content-block">
                    <h3 class="header">Anomaly detection</h3>
                    <p class="text">Find and address issues that require immediate attention before they become a problem.</p>
                </div>
                <div class="content-block">
                    <h3 class="header">Forecasting</h3>
                    <p class="text">Plan and optimize operations with insights into future events and system behavior.</p>
                </div>
                <div class="content-block">
                    <h3 class="header">Predictive maintenance</h3>
                    <p class="text">Identify when a machine is likely to fail, and take preventive measures to avoid downtime</p>
                </div>
                <div class="content-block">
                    <h3 class="header">AI Assistant</h3>
                    <p class="text">Chat with your data to get instant, AI-driven insights.</p>
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main>
            <div class="content">
                <h3 class="header">Predict failures and forecast utilization</h3>
                <div class="mobile-img">
                    <img src="/images/sentient-analytics/predict-screen.png" alt="predict screen">
                </div>
                <p class="text">For better decision making, resource planning and reducing cost business requires answers to 3 main questions: What will happen, When, and Why?</p>
                <p class="accent-text">SENTIENT ANALYTICS provides an answer</p>
                <a id="Products_TA_StartAnalysis1" href="/docs/sentient-analytics/prediction/" class="button try-pe gtm_button">Start Analysis</a>
            </div>
            <div class="image-block screenshot-block">
                <div class="image-wrapper screenshot" id="prediction">
                    <img src="/images/sentient-analytics/predict-screen.png" alt="predict screen">
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main>
            <div class="content">
                <h3 class="header">Define KPI using calculated fields</h3>
                <p class="text">Calculated fields are one of the most powerful features for KPI monitoring and prediction. Based on the input data, calculated fields allow you to run statistical functions and create new data items by applying calculations.</p>
                <a id="Products_TA_LearnMore1" href="/docs/sentient-analytics/calculations/overview/" class="button try-pe gtm_button">Learn More</a>
            </div>
            <div class="image-block">
                <div class="image-wrapper chart-wrapper pie-chart">
                    <div class="donut-chart chart-icon"></div>
                    <div class="sphere chart-icon"></div>
                    <div class="bar-chart chart-icon"></div>
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main>
            <div class="content">
                <h3 class="header">Detection Anomalies</h3>
                <div class="mobile-img">
                    <img src="/images/sentient-analytics/anomalies-screen.png" alt="anomalies screen">
                </div>
                <p class="text">Detect anomalies with automated tools based on built-in machine learning algorithms. Prioritise them and focus on real problems with anomaly scoring.</p>
                <a id="Products_TA_StartAnalysis2" href="/docs/sentient-analytics/anomaly/anomaly-detection-overview/" class="button try-pe gtm_button">Start Analysis</a>
            </div>
            <div class="image-block screenshot-block">
                <div class="image-wrapper screenshot" id="anomalies">
                    <img src="/images/sentient-analytics/anomalies-screen.png" alt="anomalies screen">
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main class="accent-circle">
            <div class="content">
                <h3 class="header">Connect Data</h3>
                <p class="text">Connect SENTIENT ANALYTICS to the SENTIENT in few clicks with automatic Topology Discovery.</p>
                <p class="text">SENTIENT ANALYTICS works with all SENTIENT products, including Open Source SENTIENT and SENTIENT Professional Edition.</p>
                <a id="Products_TA_LearnMore2" href="/docs/sentient-analytics/connect-sentient/" class="button try-pe gtm_button">Learn More</a>
            </div>
            <div class="image-block">
                <div class="image-wrapper chart-wrapper multiple-chart">
                    <div class="sphere right-sphere chart-icon"></div>
                    <div class="sphere left-sphere chart-icon"></div>
                    <div class="sphere bottom-sphere chart-icon"></div>
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main>
            <div class="content">
                <h3 class="header">Share securely</h3>
                <div class="mobile-img">
                    <img src="/images/sentient-analytics/dashboard-screen.png" alt="dashboard screen">
                </div>
                <div class="text">
                    <span>Share visualized insights with your users by:</span>
                    <ul>
                        <li>Adding them on SENTIENT dashboards</li>
                        <li>Embedding on 3rd party web sites</li>
                        <li>Exporting data into spreadsheets and images</li>
                    </ul>
                </div>
                <p class="text">All with respect to SENTIENT user permissions.</p>
                <a id="Products_TA_LearnMore3" href="/docs/sentient-analytics/embed-visuals/" class="button try-pe gtm_button">Learn More</a>
            </div>
            <div class="image-block screenshot-block">
                <div class="image-wrapper screenshot" id="dashboard">
                    <img src="/images/sentient-analytics/dashboard-screen.png" alt="dashboard screen">
                </div>
            </div>
        </main>
    </div>
    <div class="main-bg">
        <main>
            <div class="content">
                <h3 class="header">Plot and aggregate your data with any granularity</h3>
                <p class="text">See your data, like total resource consumption on any levels starting from country level and deep dive into concrete meter level. Clear dataset in real-time by grouping and filtering on dimensions that you are interested in.</p>
                <a id="Products_TA_StartAnalysis3" href="/docs/sentient-analytics/telemetry-aggregation/" class="button try-pe gtm_button">Start Analysis</a>
            </div>
            <div class="image-block accent-circle accent-circle-center">
                <div class="image-wrapper chart-wrapper line-chart">
                </div>
            </div>
        </main>
    </div>
</section>

<div id="deployment-options"></div>

<div class="main-bg accent-bg">
    <main>
        <div class="get-started">
            <div class="get-started-container">
                <h3 class="header">SENTIENT ANALYTICS Cloud</h3>
                <p class="text">Upgrade your plan to gain deeper insights into your data</p>
                <a href="https://sentient.cloud/billing" class="link">Upgrade your SENTIENT Plan</a>
            </div>
            <div class="get-started-container">
                <h3 class="header">SENTIENT ANALYTICS EU Cloud</h3>
                <p class="text">Unlock powerful analytics tools by upgrading your plan</p>
                <a href="https://eu.sentient.cloud/billing" class="link">Upgrade your SENTIENT EU Plan</a>
            </div>
            <div class="get-started-container">
                <h3 class="header">SENTIENT ANALYTICS Self-Hosted</h3>
                <p class="text">Use our step-by-step guide to install SENTIENT ANALYTICS to your server </p>
                <a href="/docs/sentient-analytics/install/installation-options/" class="link">Install SENTIENT ANALYTICS to your server</a>
            </div>
        </div>
    </main>
</div>
<div class="main-bg container-center">
    <div class="content accent-circle">
        <h1 class="main-title">SENTIENT ANALYTICS analytics</h1>
        <p class="text">Our analysis and prediction is your financial result and advance growth</p>
        <div class="btn-block">
            <a id="Products_TA_StartFree" href="/pricing/?section=sentient-analytics-options&product=sentient-analytics-self-managed&solution=sentient-analytics-pay-as-you-go" class="button accent-btn btn-open gtm_button">Start Free</a>
            <a id="Products_TA_GetADemo2" href="/products/sentient-analytics/trndz-request-demo/" class="button try-pe gtm_button">Get a demo</a>
        </div>
    </div>
</div>
